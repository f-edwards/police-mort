...
census_dat<-get_census_data(key="518b6e66ffa1857a9e4ffd5d894f2934bb06c045",
                          states=unique(coords$STname),
                          age=TRUE, sex=TRUE, census.geo="block")

#### CHECK THAT THESE INDEX
names(fe_new)<-
  # c("id", "name", "age", "gender", "race", "image", "date", 
...
fe_new<-fe_new%>%select()

##### remap county names (don't still need this with FIPS CODES!!!!)
# 
...
### manually remove duplicated case ##### CHECK ON THIS!!!!!
