...
# ... for blacks     
blk.stan.0 = stan_glmer(d.black ~ ur.code + (1|state),
...
# ... for whites 
wht.stan.0 = stan_glmer(d.white ~ ur.code + (1|state),
                      ...
                      #adapt_delta = 0.999)
# ... for latinos
lat.stan.0 = stan_glmer(d.latino ~ ur.code + (1|state),
                      prior_intercept=normal((log(0.37)-log(100000)), 2.5), #for prior intercept, based on krieger estimates
                      prior = normal(0, 2.5), #weakly informative, no difference from big urban
                      prior_covariance = decov(1, 1, 1, 1), #default
...
#save.image('bwl_fixed_mod_states.RData')


## Visuals 
# load('bwl_fixed_mod_states.RData')
blk.stan<-blk.stan.0
wht.stan<-wht.stan.0
lat.stan<-lat.stan.0
#tot.stan<-tot.stan.0
...
ggplot(p.dat3 %>% filter(ur.code == 'medium urban', real == 1), 
...
# ... or 
# ... unscaled! 
ggplot(post.blk.int2 %>% filter(ur.code == ''), 
       aes(x = reorder(state, `50%`), y = `50%`, fill = Division)) + #, #ymin = `2.5%`, ymax = `97.5%`)) + 
    geom_point(color = 'grey10', pch = 21, size = 2.5) + 
    coord_flip() + 
    facet_wrap(~ur.code) +
